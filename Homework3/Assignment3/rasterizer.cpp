...
static std::tuple<int, int, int, int> CalcAABB(const Triangle &t, int width, int height)
{
    auto *v = t.v;
    ...
    return {aabb_minX, aabb_minY, aabb_maxX, aabb_maxY};
}
...
void rst::rasterizer::rasterize_triangle(const Triangle &t, const std::array<Eigen::Vector3f, 3> &view_pos)
{
    ...
    auto v = t.v;
    auto [minX, minY, maxX, maxY] = CalcAABB(t, width, height);
    for (int i = minX; i < maxX; i++)
    {
        for (int j = minY; j < maxY; j++)
        {
            float x = i + 0.5f;
            float y = j + 0.5f;
            bool isInside = insideTriangle(x, y, v);
            if(isInside)
            {
